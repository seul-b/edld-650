...
**B2.** 
On average, students in the READ180 intervention had a score of 634 in the post-test reading comprehension, whereas students in the control group had a score of 625. The raw difference in scores seems big, <> of the standard deviation, however, we haven’t conducted a statistical test to find whether the difference is anything other than sampling idiosyncrasy.  
...
SAT<sub>i</sub> = β<sub>0</sub> + β<sub>1</sub> * Treatment<sub>i</sub> + β<sub>2</sub> * Female<sub>i</sub> + β<sub>3</sub> * FRPL<sub>i</sub> + β<sub>4</sub> * DORF<sub>i</sub> + β<sub>5</sub> * School<sub>i</sub> + ε<sub>i</sub>


Model 1 suggests that predicted attendance in the READ180 program is associated with an 11.472 point increase in reading comprehension scores (p < 0.05), indicating a positive effect of attendance on outcomes. When adding baseline covariates in Model 2, the estimated effect of attendance remains positive and statistically significant (10.236 points, p < 0.001), but the inclusion of these covariates increases the model’s explanatory power substantially, as reflected by the R-squared value of 0.736. The addition of school fixed effects in Model 3 marginally adjusts the estimated effect of attendance (10.208 points, p < 0.05) and further improves the model fit.
 
The models suggest that while individual and school-level characteristics are important, attendance at the READ180 program itself is a key predictor of reading comprehension achievement among students. The results here differ from the raw comparison in B2 in that we didn’t examine statistical significance in B2. In addition, the results here provide an adjusted measure of the READ180 program's effect, accounting for individual characteristics and school-level influences. While B2's raw difference in scores does not account for these factors, the estimates provided here offer a more nuanced understanding of the relationship between READ180 attendance and reading outcomes. The statistical significance and control for confounding baseline variables in the regression make our model estimates more precise. 
...
Attendance<sub>i</sub> = π<sub>0</sub> + π<sub>1</sub> * Treatment<sub>i</sub> + π<sub>2</sub> * Female<sub>i</sub> + π<sub>3</sub> * FRPL<sub>i</sub> + π<sub>4</sub> * DORF<sub>i</sub> + π<sub>5</sub> * School<sub>i</sub> + δ<sub>i</sub> 

Second stage equation:

SAT<sub>10i</sub> = β<sub>0</sub> + β<sub>1</sub> * &#773;Attendance<sub>i</sub> + β<sub>2</sub> * Female<sub>i</sub> + β<sub>3</sub> * FRPL<sub>i</sub> + β<sub>4</sub> * DORF<sub>i</sub> + β<sub>5</sub> * School<sub>i</sub> + ε<sub>i</sub>

In the first-stage model, there was a strong relationship between the assignment to the treatment and attendance of the READ180 program (F = ). We found that there was a positive impact of the READ180 program on student’s comprehension scores; on average, a 100 percent attendance in the seven-month program increased student scores by 10 points (p < 0.05) compared to not attending at all. 
