...
$$Pr(EL \space Program) = \beta_0 + \beta_1*PreLAS + \beta_2*EBRS + \beta_3*SES + \beta_4*Rurality + \beta_5*Gender + \beta_6*Latinx + \epsilon_i$$
...
ggplot(dat, aes(p_score, fill = as.factor(elprgm))) + 
  ...
  labs(x = "Probability of Attending EL programs",
       fill = "Attended EL Program",
...
df_cem %>% 
  ...
  ggplot(., aes(p_score, fill = as.factor(elprgm), color = as.factor(elprgm))) + 
  geom_density(alpha=0.1) + 
  theme_pander(base_size = 12) + 
  xlab("Probability of EL Identification") +
  labs(color = "Identified as EL",
       fill = "Identified as EL")
```
...
t_tests <- lapply(vars, function(var_name) {
  t.test(reformulate("elprgm", response = var_name), data = combined)
})
stats <- data.frame(
  Variable = vars,
